...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
    ...
    return {
      ...
      openGraph: {
        title: `${blog?.title} | Broadersight`,
        description: `${blog?.metaDescription}`,
        url: `https://broadersight.com/blog/${slug}`,
...
async function Page({ params, searchParams }: { params: { slug: string }, searchParams: { filter?: string } }) {
    const {slug} = await params;
    ...
    const hashtagsArray = blog.tags?.map(tag => encodeURIComponent(tag.replace(/\s+/g, ''))) || [];
    const hashtags = hashtagsArray.join(',');

    const socialPlatforms = [
        {
          name: "WhatsApp",
          url: `https://api.whatsapp.com/send?text=${encodeURIComponent(`*${blog.title}..*

📖 *Check it out 👇*  
https://broadersight.com/blog/${slug}`)}`,
          svg: (
        ...
        {
          name: "LinkedIn",
          url: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`Check out this Insightful Blog 👇
            ${blog.title}
            https://broadersight.com/blog/${slug}`)}`,
          svg: (
  ...
  `Check out this blog 👇\n${blog.title} \nhttps://broadersight.com/blog/${slug}${hashtags ? `&hashtags=blog,${hashtags}` : '&hashtags=blog'}`
)}`,
          svg: (
        ...
        {
          name: "Facebook",
          url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`Check out this Insightful Blog 👇
            ${blog.title}
            https://broadersight.com/blog/${slug}${hashtags ? `&hashtags=blog,${hashtags}` : '&hashtags=blog'}`)}`,
          svg: (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    ...
    <article className='mx-auto 2xl:w-[1100px] xl:w-[1000px] lg:w-[900px] max-lg:w-[90%]  max-sm:mx-4 flex justify-center py-20 pb-6 max-sm:py-16 max-sm:pb-0' itemScope itemType="https://schema.org/BlogPosting">
        {/* Main Content  */}
        <header className='flex flex-col gap-6 2xl:w-[85%] xl:w-[100%] w-[100%] max-md:w-full'>
            <h1 className='text-[40px] max-sm:text-3xl leading-[50px] text-zinc-700 dark:text-purple-200 font-bold font-montserratAlt'>
                 {blog.title}
            </h1>
            <h2 className='text-base max-sm:text-sm max-lg:pr-4 text-zinc-500 dark:text-zinc-300 font-medium font-montserrat pr-16 '>
            ...
            <div className='flex flex-col gap-4'>
                {blog.content.map((block, index)=>{
                    switch (block.type.toLowerCase()) {
                        ...
                        case "image":
                          return <ImageDisplay key={index} value={block.value} title={blog.title} />;
                        case "youtube video":
